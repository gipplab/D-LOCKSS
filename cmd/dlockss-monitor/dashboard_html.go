        ...
        function updateDashboard() {
            ...
            fetch(url).then(r=>r.json()).then(data => {
                ...
                tbody.innerHTML = nodes.map(n => {
                    ...
                    return '<tr data-peer-id="' + escapeHtml(n.peer_id.toLowerCase()) + '"><td><button class="btn-text alias-edit-btn" onclick="editAlias(\'' + peerIdEscaped + '\', \'' + aliasEscaped + '\', this)">EDIT</button></td><td class="peer-id-cell"><div class="alias-display-container"><div style="font-weight:600;">' + (aliasHtml || peerIdHtml.slice(0,12) + '...') + '</div><div style="font-size:0.8em; color:#666;">' + peerIdHtml + '</div></div></td><td>' + (m.region ? regionHtml : '<span style="color:#999">LOCATING...</span>') + '</td><td><span class="shard-badge">' + shardHtml + '</span></td><td>' + n.peers_in_shard + '</td><td>' + n.storage.pinned_files + '</td><td>' + n.storage.known_files + '</td><td>' + Math.floor(n.uptime_seconds/60) + 'm</td><td><span class="status-text status-online">[ACTIVE]</span> ' + lastSeen + 's ago</td></tr>';
                }).join('');
                restoreEditingState();
            });
        }
